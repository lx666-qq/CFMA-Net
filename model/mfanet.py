...
class CMAD(nn.Module):
    ...
    def forward(self, query_feat, support_feat, merge_feat, h, w, return_intermediate=False):
        ...
        x_fused = self.pma_high(
            x_fused, F.interpolate(
                fused_feat, size=x_fused.shape[2:], mode='bilinear', align_corners=False
            )
        )
        ...
        if return_intermediate:
            return seg_out, {'support_enhanced': support_enhanced.detach()}
        return seg_out
...
class CFMANet(nn.Module):
    ...
    def forward(self, x, s_x=None, s_y=None, y=None, return_vis=False, return_pma_analysis=False):
        ...
        for i in range(self.shot):
            ...
            supp_feat_list.append(supp_feat.detach())
        ...
        if return_vis or return_pma_analysis:
            seg_out, cmad_inter = self.cmad(
                query_feat=query_enhanced, support_feat=prototype,
                merge_feat=merge_feat, h=h, w=w, return_intermediate=True
            )
        else:
            seg_out = self.cmad(
                query_feat=query_enhanced, support_feat=prototype,
                merge_feat=merge_feat, h=h, w=w
            )
        ...
        if not self.training:
            ...
            if return_vis or return_pma_analysis:
                vis_dict = {
                    ...
                    'supp_feat': supp_feat_list[0] if supp_feat_list else None,
                    ...
                    'fdpg': fdpg_vis,
                    'pma': pma_vis,
                    'support_enhanced': cmad_inter['support_enhanced'],  # 新增
