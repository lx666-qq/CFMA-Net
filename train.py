    ...
    for batch_idx, (input, target, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
        ...
        if main_process() and do_viz and vis_dict is not None:
            ...
            for sample_idx in range(batch_size):
                ...
                try:
                    raw_sample_vis = slice_vis_dict(vis_dict, sample_idx, batch_size)

                    def save_feat_colormap(tensor, path, is_single_channel=False):
                        """将特征保存为彩色热力图"""
                        if tensor is None:
                            return
                        feat = tensor[0].detach().cpu().float().numpy()

                        if is_single_channel:
                            # corr_mask: (1, H, W) -> (H, W)
                            energy = feat[0]
                        else:
                            # 多通道特征: (C, H, W) -> 计算 L2 范数 -> (H, W)
                            energy = np.linalg.norm(feat, ord=2, axis=0)

                        # 归一化到 [0, 1]
                        e_min, e_max = energy.min(), energy.max()
                        energy = (energy - e_min) / (e_max - e_min + 1e-8)

                        # 应用伪彩色映射
                        cmap = plt.get_cmap('jet')
                        colored = cmap(energy)[:, :, :3]  # 去掉 Alpha 通道
                        colored = (colored * 255).astype(np.uint8)
                        colored = cv2.cvtColor(colored, cv2.COLOR_RGB2BGR)
                        cv2.imwrite(path, colored)

                    # 1. Support Enhanced
                    if raw_sample_vis.get('support_enhanced') is not None:
                        save_feat_colormap(
                            raw_sample_vis['support_enhanced'],
                            os.path.join(vis_dir, f'{save_idx:04d}_support_enhanced.png')
                        )

                    # 2. Query Enhanced
                    if raw_sample_vis.get('query_enhanced') is not None:
                        save_feat_colormap(
                            raw_sample_vis['query_enhanced'],
                            os.path.join(vis_dir, f'{save_idx:04d}_query_enhanced.png')
                        )

                    # 3. Corr Mask (单通道，直接映射)
                    if raw_sample_vis.get('corr_mask') is not None:
                        save_feat_colormap(
                            raw_sample_vis['corr_mask'],
                            os.path.join(vis_dir, f'{save_idx:04d}_corr_mask.png'),
                            is_single_channel=True
                        )
                    # 定义一个辅助函数：将特征转为彩色热力图
                    def save_feat_heatmap(tensor, path):
                        if tensor is None: return
                        # 取第一个样本，转为 CPU numpy
                        # shape: (C, H, W)
                        feat = tensor[0].detach().cpu().float().numpy()

                        # 1. 计算通道维度的 L2 范数 (能量)，得到 (H, W)
                        # 这样可以将多通道特征压缩为单通道响应图
                        energy = np.linalg.norm(feat, ord=2, axis=0)
                        ...
                        # 3. 应用 colormap 转为彩色图 (例如 'viridis' 或 'jet')
                        # matplotlib colormap 输出 (H, W, 4) RGBA
                        cmap = plt.get_cmap('viridis')
                        colored_energy = cmap(energy)[:, :, :3]  # 去掉 Alpha 通道

                        # 4. 转为 0-255 uint8 并保存
                        colored_energy = (colored_energy * 255).astype(np.uint8)
                        # 注意：cv2 保存是 BGR，但这里我们是 RGB，转换一下更符合图片格式
                        colored_energy = cv2.cvtColor(colored_energy, cv2.COLOR_RGB2BGR)
                        cv2.imwrite(path, colored_energy)

                    # 保存 Query Feature
                    if raw_sample_vis.get('query_feat') is not None:
                        save_feat_heatmap(raw_sample_vis['query_feat'],
                                          os.path.join(vis_dir, f'{save_idx:04d}_query_feat_color.png'))

                    # 保存 Support Feature
                    if raw_sample_vis.get('supp_feat') is not None:
                        save_feat_heatmap(raw_sample_vis['supp_feat'],
                                          os.path.join(vis_dir, f'{save_idx:04d}_supp_feat_color.png'))
