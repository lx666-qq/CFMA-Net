...
def make_dataset(data_root=None, data_list=None, train_class=None):
...
class SemData(Dataset):
    def __init__(self, shot=1, data_root=None, data_list=None, transform=None, mode='train'):
        ...
        self.train_class = list(range(1, 4))  # 1 2 3 (4 remove)
        self.val_class = list(range(5, 11))  # 4 5 6 7 8 9 10


        if self.mode == 'train':
            self.img_gt_class_list, self.class_img_gt_dict = make_dataset(data_root, data_list, self.train_class)
            assert len(self.class_img_gt_dict.keys()) == len(self.train_class)

        elif self.mode == 'val':
            self.img_gt_class_list, self.class_img_gt_dict = make_dataset(data_root, data_list, self.val_class)
