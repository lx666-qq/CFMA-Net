...
class Resize(object):
    ...
    def __call__(self, image, label):
        image_resized = cv2.resize(image, dsize=(int(self.h), int(self.w)), interpolation=cv2.INTER_LINEAR)

        label_resized = cv2.resize(label.astype(np.float32), dsize=(int(self.h), int(self.w)),
